...
def get_number_of_lines():
    while True:
        lines = input("Enter the no of lines to bet from (1 -" +str(MAX_LINES) +" )? "  )
        if lines.isdigit():
            lines = int(lines)
...
def spin(balance):
    ...
    print(f"You are betting on ${bet} in {lines} lines. Total bet is: {total_bet}")
    print(balance, lines)
...
def main():
    ...
    while True:
        ...
        answer = input(f"Press enter to spin ( q to quit)")
        if answer == "q":
            break
